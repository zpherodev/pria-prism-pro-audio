...
export const HorizontalPiano: React.FC<HorizontalPianoProps> = ({
  ...
  const incrementOctave = () => {
    if (startOctave < 8) { // Standard piano has 88 keys (A0 to C8)
  ...
  const handleDrop = (e: React.DragEvent, midiNote: number) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (file.type.startsWith('audio/')) {
        ...
        toast.success(`Sound mapped to ${keyNames[midiNote % 12]}${Math.floor(midiNote / 12)}`);
      } else {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Map computer keyboard to piano keys (Z-M for white keys, S-J for black keys)
  ...
  return (
    <div className="horizontal-piano-container">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-sm font-medium">MIDI Mapping - Octave {startOctave + 4}</h3>
        ...
        {keyNames.map((note, i) => {
          ...
          return (
            <div 
              ...
              className={`
                piano-key relative 
                ${isBlackKey[i] ? 'black-key' : 'white-key'} 
                ${isBlackKey[i] ? 'z-10 h-3/5 -mx-2 w-4' : 'h-full flex-1 min-w-8'} 
                ${draggedOverKey === midiNote ? 'border-2 border-amber-400' : ''}
                ${currentPlayingNote === midiNote ? 'bg-emerald-500' : 
                  isBlackKey[i] ? 'bg-zinc-800' : 'bg-white'}
                ${mappedSound ? 'mapped' : ''}
                transition-colors
              `}
            ...
            >
              <div className={`key-label text-xs absolute bottom-1 ${isBlackKey[i] ? 'text-white' : 'text-zinc-800'} text-center w-full`}>
                {note}
              </div>
              {mappedSound && (
                <div className={`mapped-indicator absolute top-1 left-1/2 transform -translate-x-1/2 text-xs px-1 py-0.5 rounded ${isBlackKey[i] ? 'bg-zinc-600' : 'bg-zinc-200'}`}>
                  <Music className={`h-3 w-3 ${isBlackKey[i] ? 'text-white' : 'text-zinc-800'}`} />
                </div>
              )}
            </div>
          );
      ...
      <div className="text-xs text-zinc-400 text-center">
        Drag audio files onto keys to map sounds. Click to preview.
      </div>
    </div>
  );
};
